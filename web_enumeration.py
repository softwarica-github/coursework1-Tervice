...
def read_directory_list(directory):
    ...
    try:
        ...
        output_field.insert(tk.END, f"{directory} does'nt exist.\n")
        output_field.update()
        return []
...
first_frame = tk.Frame(root, bg="#0cbd3c")
first_frame.grid(row=0, column=0, rowspan=12, columnspan=12, sticky="nsew")
for i in range(12):
    first_frame.rowconfigure(i, minsize=50)
for i in range(12):
...
project_text = ttk.Label(first_frame, text="Web Enumeration Tool", font=("Helvetica", 30), foreground="#ffff00", background="#333333")
project_text.grid(row=3, column=3, columnspan=3, pady=20, padx=40)

name_text = ttk.Label(first_frame, text="By: Shasank Pyakurel", font=("Helvetica", 10), foreground="#ffff00", background="#333333")
name_text.grid(row=11, column=5, columnspan=8, pady=(10,150), padx=10 ,sticky='n')
...
directory_scan_button = tk.Button(first_frame, text="Directory Scan", command=directory_scan, font=("Helvetica", 13), background="#00b4d8", 
                        foreground="#ffff00", relief="raised", bd=3, activebackground="#144552",activeforeground="#1985a1")
directory_scan_button.grid(row=5, column=3, columnspan=3,sticky="s",  pady=20)
...
gather_info_button = tk.Button(first_frame, text="Get Info", command=gather_info, font=("Helvetica", 13), background="#00b4d8", 
                        foreground="#ffff00", relief="raised", bd=3, activebackground="#144552",activeforeground="#c5c3c6")
gather_info_button.grid(row=6, column=3, columnspan=3,sticky="s",  pady=20)
...
exit_button = tk.Button(first_frame, text="Exit", command=root.quit, font=("Helvetica", 13), background="#00b4d8", 
                        foreground="#ffff00", relief="raised", bd=3, activebackground="#144552",activeforeground="red")
exit_button.grid(row=7, column=3, columnspan=3, sticky="s",  pady=20)
...
style.configure("Round.TEntry", fieldbackground="#ffffff", background="transparent", 
                 ...
                 highlightcolor="#c9b678", highlightbackground="#c9b678", 
                 borderradius=10)


directory_scan_frame = tk.Frame(root, bg="#333333")

sign_up_text = ttk.Label(directory_scan_frame, text="Directory Scan", font=("Travelast", 20), foreground="#ffff00", background="#333333")
sign_up_text.grid(row=0, column=3, columnspan=1, pady=30, padx=50, sticky="nw")


'''sign up frame ---- first name lable and entry '''
ip_address_label = tk.Label(directory_scan_frame, text="Enter URL", font=("Helvetica", 12), foreground="#ffff00", background="#333333")
ip_address_entry = ttk.Entry(directory_scan_frame, width=35, font=("Helvetica", 12), foreground="#333333", style="Round.TEntry", background='gray')
...
output_field = tk.Text(directory_scan_frame, height=20, width=75, background="black", font=("Helvetica", 12), foreground="white", highlightthickness=0)
...
search_entry = ttk.Entry(search_container, width=20, background="black", font=("Helvetica", 12), foreground="black", style="Round.TEntry")
...
log_in_text = ttk.Label(info_frame, text="Get Information", font=("Travelast", 25), foreground="#ffff00", background="#333333")
...
url_label = tk.Label(info_frame, text="Enter URL", font=("Helvetica", 12), foreground="#ffff00", background="#333333")
url_entry = ttk.Entry(info_frame, width=35, font=("Helvetica", 12), foreground="#333333", style="Round.TEntry", background='gray')
...
output_field_info = tk.Text(info_frame, height=20, width=70, background="black", font=("Helvetica", 12), foreground="white", highlightthickness=0)
...
save_button = tk.Button(info_frame, text="Save", command=save_output, font=("Helvetica", 8), background="#00b4d8", 
                        foreground="#ffff00", relief="raised", bd=3, activebackground="#144552",activeforeground="red")
